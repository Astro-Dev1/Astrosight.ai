...
import {YearlyHoroscopeFull} from '../../../components/DailySignArticle';
...
const  fetchZodiacOverview=(async (sign, period = "yearly", language = "en") =>{
 ...
 useEffect(() => {
    ...
    const results = [];
      ...
      for (const sign of signs) {
        ...
        try {
          const overview = await fetchZodiacOverview(sign, "yearly", language);
  ...
  const fetchDailyHoroscope = async () => {
    ...
    try {
      ...
      const response = await getDailyHoroscope({
        type: "yearly", // Fetch actual period-specific data (daily, weekly, monthly, yearly)
        lang: language === 'hi' ? 'hn' : language,
        sign: capitalizedSign,
        date: formattedDate
      });
  ...
  canonical={`https://astrosight.ai/horoscope/${sign}${currentPeriod !== 'yearly' ? '/' + currentPeriod : ''}`}
...
/>
      <Head>
        ...
        <link rel="canonical" href={`https://astrosight.ai/horoscope/${sign}${currentPeriod !== 'yearly' ? '/' + currentPeriod : ''}`} />
        
  {/* ...other meta or SEO tags... */}
  <script
    ...
    dangerouslySetInnerHTML={{
      __html: JSON.stringify({
        ...
        "url": `https://astrosight.ai/horoscope/yearly-horoscope/${sign}`,
        "provider": {
          "@type": "Organization",
          "name": "AstroSight",
            ...
            <div className="mt-12   space-y-8">
              <div className="mb-8 ">
                ...
                <div className="bg-white  mx-auto px-4 sm:px-6 lg:px-4">
                        ...
                        <YearlyHoroscopeFull sign={capitalizedSign} />
</div> 
              </div>
