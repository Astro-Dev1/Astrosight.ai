...
const PeriodSelector = ({ currentPeriod = 'yearly', sign }) => {
...
export default function HoroscopePeriodPage({ initialHoroscopeData, signParam, initialLanguage }) {
  ...
  // Fetch yearly horoscope if missing client-side (fallback)
  useEffect(() => {
  ...
  async function fetchDaily() {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch yearly horoscope');
      setHoroscope({
        text: `This year brings new opportunities for ${capitalizedSign}. Stay focused and trust your intuition.`,
        sections: {},
      });
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <SEOHead
        title={`${capitalizedSign} Yearly Horoscope | AstroSight`}
        description={`Discover ${capitalizedSign}'s yearly horoscope. Get predictions for love, career, health, and more at AstroSight.`}
        keywords={`${capitalizedSign} yearly horoscope, zodiac yearly predictions, astrology yearly insights`}
        canonical={`https://astrosight.ai/horoscope/yearly-horoscope/${sign.toLowerCase()}`}
      ...
      <Head>
        <title>{`${capitalizedSign} Yearly Horoscope | AstroSight`}</title>
        <meta name="description" content={`Discover your ${capitalizedSign} yearly horoscope. Get predictions for love, career, health, and more at AstroSight.`} />
        <link rel="canonical" href={`https://astrosight.ai/horoscope/yearly-horoscope/${sign.toLowerCase()}`} />
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "WebPage",
              "name": `${capitalizedSign} Yearly Horoscope | AstroSight`,
              "headline": `${capitalizedSign} Yearly Horoscope`,
              "description": `Discover ${capitalizedSign}'s yearly horoscope. Get predictions for love, career, health, and more at AstroSight.`,
              "url": `https://astrosight.ai/horoscope/yearly-horoscope/${sign.toLowerCase()}`,
              "provider": {
                "@type": "Organization",
                "name": "AstroSight",
      ...
      <div className="flex flex-col min-h-screen bg-[#FFF4EA] relative pb-16 font-inter">
        ...
        <div className="flex-1 pt-16">
          <div className="px-4 pb-20 max-w-5xl mx-auto">
            ...
            {/* Period Selector */}
            <PeriodSelector currentPeriod="yearly" sign={sign.toLowerCase()} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
                {zodiacSignsList.map((zodiac) => (
                  <div
                    ...
                    onClick={() => router.push(`/horoscope/yearly-horoscope/${zodiac.toLowerCase()}`)}
                  >
                    <Image
