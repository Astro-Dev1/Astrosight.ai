...
import { DailyHoroscopeFull } from '../../../components/DailySignArticle';
...
const PeriodSelector = ({ currentPeriod = 'today', sign }) => {
...
export default function HoroscopePeriodPage({ initialHoroscopeData, signParam, initialLanguage }) {
  ...
  // Fetch daily horoscope if missing client-side (fallback)
  useEffect(() => {
  ...
  async function fetchDaily() {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch today horoscope');
      setHoroscope({
        text: `Today brings new opportunities for ${capitalizedSign}. Stay focused and trust your intuition.`,
        sections: {},
      });
  ...
  return (
    <>
      ...
      <SEOHead
        title={`${capitalizedSign} Today Horoscope | AstroSight`}
        description={`Discover ${capitalizedSign}'s today horoscope. Get predictions for love, career, health, and more at AstroSight.`}
        keywords={`${capitalizedSign} today horoscope, zodiac daily predictions, astrology daily insights`}
        canonical={`https://astrosight.ai/horoscope/today-horoscope/${sign.toLowerCase()}`}
      ...
      <Head>
        <title>{`${capitalizedSign} Today Horoscope | AstroSight`}</title>
        <meta name="description" content={`Discover your ${capitalizedSign} today horoscope. Get predictions for love, career, health, and more at AstroSight.`} />
        <link rel="canonical" href={`https://astrosight.ai/horoscope/today-horoscope/${sign.toLowerCase()}`} />
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "WebPage",
              "name": `${capitalizedSign} Today Horoscope | AstroSight`,
              "headline": `${capitalizedSign} Today Horoscope`,
              "description": `Discover ${capitalizedSign}'s today horoscope. Get predictions for love, career, health, and more at AstroSight.`,
              "url": `https://astrosight.ai/horoscope/today-horoscope/${sign.toLowerCase()}`,
              "provider": {
                "@type": "Organization",
                "name": "AstroSight",
      ...
      <div className="flex flex-col min-h-screen bg-[#FFF4EA] relative pb-16 font-inter">
        ...
        <div className="flex-1 pt-16">
          <div className="px-4 pb-20 max-w-5xl mx-auto">
            ...
            {/* Period Selector */}
            <PeriodSelector currentPeriod="today" sign={sign.toLowerCase()} />

            {/* Overview Card */}
            <Card className="bg-white p-5 rounded-xl shadow-lg mb-6 border border-gray-200">
              <h3 className="text-black text-xl font-semibold mb-3">Daily Overview</h3>
